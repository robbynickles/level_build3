...
import magnet
from magnify.magnify import clear_magnifier, magnify_touch

from libs.game.physics_interface.game_objects.static_line import UserStaticLine
...
def drawline( self, touch, touch_stage, magnetize, curve=False ):
    ...
    if touch_stage == 'touch_up':
        ...
        if self.line_progress:
            ...
            # Don't draw anything if the touch is too quick.
            if not touch.time_end - touch.time_start <= self.QUICK:
                if curve:
...
def editline( self, touch, touch_stage, magnetize ):
    ...
    if touch_stage == 'touch_down':
        ...
        if isinstance( self.target_line, UserStaticLine ):

            # Move the endpoint the touch is closest to.
            if distance( touch.pos, self.target_line.get_start() ) < \
               distance( touch.pos, self.target_line.get_end() ):
                self.move_start = True
            else:
                self.move_end   = True

        else: # self.target_line is a UserStaticCurve
    ...
    if touch_stage == 'touch_move':
        ...
        # and the offset hasn't already been changed,
        # offset the touch positions to be more visible.
        if self.y_off == 0 and distance( (0,0), touch.dpos ) >= SIGNIFICANT:
